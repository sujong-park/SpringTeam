 ...
 * 현재는 create.html에 포함된 스크립트에서 호출됩니다.
 */
function addPetSelection() {
    const container = document.getElementById('additionalPetsContainer');

    // 이미 선택된 펫들을 가져와 중복 선택 방지
    const selectedPetIds = Array.from(document.querySelectorAll("input[name='additionalPetIds']")).map(input => input.value);
    const availablePets = /*[[${memberPets}]]*/ []; // 서버에서 주입된 memberPets 사용

    const filteredPets = availablePets.filter(pet => !selectedPetIds.includes(pet.petId.toString()));
